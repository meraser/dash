...
myGlobal.saveT = function (id) {
    let tt = id.split('_');
    myGlobal.currentCheck = (tt[1] == 'bt1');
...
myGlobal.makeList = function (sub, obj, page) {
    ...
    for (let i in issues) {
        ...
        let row = $('<tr  />');
        let tempLink = (i.indexOf('TVCSISSUE') > -1) ? '"http://hlm.lge.com/qi/browse/' : '"http://hlm.lge.com/issue/browse/';
        cntt++;
...
myGlobal.loadIssue = function (i, data, deferred) {
    ...
    if (i == 'dev') {
        for (let i in arr) {
            ...
            myGlobal.allIssue[tempKey].Description = ar.description ? ar.description : '';
            myGlobal.allIssue[tempKey].issueDes = ar.customfield_16500 ? ar.customfield_16500 : '';
            myGlobal.allIssue[tempKey].Assignee = ar.assignee ? ar.assignee.displayName.split(' ')[0] : 'Empty';
            myGlobal.allIssue[tempKey].Labels = ar.labels;
            myGlobal.allIssue[tempKey].Area = ar.components ? ar.components[0].description : 'Empty';
            myGlobal.allIssue[tempKey].Resol = ar.resolution ? ar.resolution.name : 'Empty';
            myGlobal.allIssue[tempKey].Ftype = ar.customfield_16503 ? ar.customfield_16503.value : 'Empty';
            myGlobal.allIssue[tempKey].Stype = ar.customfield_16502 ? ar.customfield_16502.value : 'Empty';
        ...
        for (let i in arr) {
            ...
            myGlobal.etcIssue[tempKey].Assignee = ar.customfield_14503 ? ar.customfield_14503[0].displayName.split(' ')[0] : 'Empty';
        }
        myGlobal.dResolve(deferred);
